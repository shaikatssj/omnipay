...
class RocketDriver implements PaymentDriverInterface
{
    ...
    public function initiatePayment(Invoice $invoice, array $settings): array
    {
        ...
        $receivedBDT = floatval($meta['rocket_received_amount_bdt'] ?? 0);
        ...
        return [
            ...
            'instructions' => 'Send Send-Money to the Rocket personal number and input the Transaction ID.',
    ...
    public function verifyPayment(Invoice $invoice, array $settings, array $requestData): array
    {
        ...
        if ($transaction) {
            ...
            $meta['rocket_payments'][] = [
                'trxid' => $trx_id,
                'amount' => $paidAmountBDT,
                'time' => time()
            ];
            $meta['rocket_received_amount_bdt'] = $newReceivedBDT;
    ...
    public function refund(Invoice $invoice, array $settings, array $refundData): array
    {
        return [
            'status' => 'manual',
            'message' => 'Manual MFS refund required. Please refund the customer ' . ($refundData['amount'] ?? $invoice->amount) . ' BDT manually via Rocket.'
        ];
    }
}
