...
class TapDriver implements PaymentDriverInterface
{
    ...
    public function initiatePayment(Invoice $invoice, array $settings): array
    {
        ...
        $receivedBDT = floatval($meta['tap_received_amount_bdt'] ?? 0);
        ...
        return [
            ...
            'instructions' => 'Send Send-Money to the Tap personal number and input the Transaction ID.',
    ...
    public function verifyPayment(Invoice $invoice, array $settings, array $requestData): array
    {
        ...
        if ($transaction) {
            ...
            $meta['tap_received_amount_bdt'] = $newReceivedBDT;
                ...
                try {
                    Mail::send([], [], function ($message) use ($merchantEmail, $invoice, $trx_id, $totalBDT, $receivedBDT) {
                        $message->to($merchantEmail)
                            ->subject("Tap Verification Pending - Invoice #{$invoice->invoice_id}")
                            ->html("
                                <h3>Tap Verification Waiting</h3>
                                <p>Hello,</p>
                                <p>A customer submitted a Tap Transaction ID for manual verification. It has not yet been synced by the transaction reader app.</p>
                                <ul>
    ...
    public function refund(Invoice $invoice, array $settings, array $refundData): array
    {
        return [
            'status' => 'manual',
            'message' => 'Manual MFS refund required. Please refund the customer ' . ($refundData['amount'] ?? $invoice->amount) . ' BDT manually via Tap.'
        ];
    }
}
