...
class NagadDriver implements PaymentDriverInterface
{
    ...
    public function initiatePayment(Invoice $invoice, array $settings): array
    {
        ...
        $receivedBDT = floatval($meta['nagad_received_amount_bdt'] ?? 0);
    ...
    public function verifyPayment(Invoice $invoice, array $settings, array $requestData): array
    {
        ...
        if ($transaction) {
            ...
            $meta['nagad_received_amount_bdt'] = $newReceivedBDT;
                ...
                try {
                    Mail::send([], [], function ($message) use ($merchantEmail, $invoice, $trx_id, $totalBDT, $receivedBDT) {
                        $message->to($merchantEmail)
                            ->subject("Nagad Verification Pending - Invoice #{$invoice->invoice_id}")
                            ->html("
                                <h3>Nagad Verification Waiting</h3>
                                <p>Hello,</p>
                                <p>A customer submitted a Nagad Transaction ID for manual verification. It has not yet been synced by the transaction reader app.</p>
                                <ul>
    ...
    public function refund(Invoice $invoice, array $settings, array $refundData): array
    {
        return [
            'status' => 'manual',
            'message' => 'Manual MFS refund required. Please refund the customer ' . ($refundData['amount'] ?? $invoice->amount) . ' BDT manually via Nagad Personal.'
        ];
    }
}
